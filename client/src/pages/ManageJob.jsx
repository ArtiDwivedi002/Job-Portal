...
const ManageJob = () => {
  ...
  const fetchCompanyJobs = async () => {
    try {
      const { data } = await axios.get(
        backendUrl + '/api/company/list-job',
  ...
  // ✅ FIX: Move this outside fetchCompanyJobs
  ...
  return jobs ? jobs.length === 0 ? (<div className='flex items-center justify-center h-[70vh]'>
    ...
    <div className='container p-4 max-w-5xl '>
      ...
      <div className='mt-4 flex justify-end'>
        <button onClick={() => navigate('/dashboard/add-job')} className='bg-black text-white py-2 px-4
         rounded'>Add New Job</button>
      </div>
    </div>
  ) : <Loading />
