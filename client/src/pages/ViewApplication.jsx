...
const ViewApplication = () => {
  ...
  //Functionto fetch company Job applications data
  ...
  return applicants ? applicants.length === 0 ? (<div className='flex items-center justify-center h-[70vh]'>
    <p className='text-xl sm:text-2xl'>No Application is Available</p>
  </div>) : (
          ...
          <tbody>
            {applicants.filter(item => item.jobId && item.userId).map((applicant, index) => (
              <tr key={index} className='text-gray-700 '>
                <td className='py2 px-4 border-b text-center'>{index + 1}</td>
                ...
                <td className='py2 px-4 border-b max-sm:hidden'>{applicant.jobId.title}</td>
                <td className='py2 px-4 border-b max-sm:hidden'>{applicant.jobId.location}</td>
